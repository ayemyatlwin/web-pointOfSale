...
const YearlyFinance = () => {
  ...
  const getYearlyData=useGetYearlyFinanceInfoQuery({token,currentPage,year:getYear});
    ...
    <>
      ...
      <div className="py-5 pb-3 ">
        <div className=" flex justify-between">
          ...
          {selectedDate !== null ? `${getYear} Sales Overview`: `This Month sales Overview`}
          </h2>
          <div className="flex gap-3">
            <DropDownBtn />
            <YearPickers selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
          </div>
        </div>
      </div>
      ...
      <main className="border border-[#3f4245] rounded-sm mt-7">
        <table className="w-full text-sm text-center text-[#f5f5f5]">
          ...
          {getAllYearlyData && getAllYearlyData.length === 0 ? (
            ...
            <tbody className="text-[#f5f5f5]">
              {getAllYearlyData?.map((yearlyData, index) => {
                return (
                  <tr key={index} className="border-b border-[#3f4245]">
                    ...
                    <td className="px-6 py-4">{yearlyData?.tax.toFixed(2)}</td>
                    <td className="px-6 py-4">
      ...
      {getAllYearlyData && getAllYearlyData.length === 0 ? (
        ...
        <div className="flex gap-3 mb-2 mt-5 border border-[#3f4245] rounded-md">
          <button className="border-r border-[#3f4245] flex flex-col w-[7rem] py-2 px-2 ">
            ...
            <span className="text-sm self-end">{getAllYearlyData?.length}</span>
          </button>
          ...
          <button className="border-r border-[#3f4245]  flex flex-col w-[7rem] py-2 px-2 ">
            ...
            <span className="text-sm self-end">{totals?.total_cash.toFixed(2)}</span>
          </button>
          <button className="border-r border-[#3f4245] flex flex-col w-[7rem] py-2 px-2 ">
            <span className="text-xs self-end text-[#8AB4F8] ">Total Tax</span>
            <span className="text-sm self-end">{totals?.total_tax.toFixed(2)}</span>
          </button>
          <button className=" flex flex-col w-[7rem] py-2 px-2 ">
            <span className="text-xs self-end text-[#8AB4F8] ">Total </span>
            <span className="text-sm self-end">{totals?.total.toFixed(2)}</span>
          </button>
        </div>
